...
void mqttCallback(char *topic, byte *payload, unsigned int length)
{
  ...
  String topicandpayload;
    //重新组装成json格式，方便后面处理。
  topicandpayload = "{\"Topic\":\"";
  topicandpayload.concat(topic);
  topicandpayload.concat("\",\"Value\":");
  for (int i = 0; i < length; i++)
  {
    topicandpayload.concat((char)payload[i]);
  }
  topicandpayload.concat("");
  topicandpayload.concat("}");
  Serial.println(topicandpayload);
...
String randomPropertyData()
{
  ...
  objTmeperature["id"] = "priThreePhaseVoltage";
  ...
  objBrightness["id"] = "priThreePhaseCurrent";
  ...
  category_light["id"] = "priThreePhasePower";
  ...
  category_humidity["id"] = "priThreePhaseEnergy";
  randInt = random(0, 100);
...
void processFunction(String msg)
{
  ...
  for (JsonObject object : doc.as<JsonArray>())
  {
    ...
    printMsg("开关 Switch" + (String)strcmp(id, "Switch"));
...
void processEvent()
{
  ...
  JsonObject objTmeperature = doc.createNestedObject();
  objTmeperature["id"] = "secBMSStatus";
  objTmeperature["value"] = "40";
  objTmeperature["remark"] = "副边BMS状态";

  JsonObject objException = doc.createNestedObject();
  objException["id"] = "secChargeStatus";
  objException["value"] = "60";
  objException["remark"] = "副边充电状态";
