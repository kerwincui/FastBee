...
#elif defined(MBEDTLS_OS_FREERTOS)

static void threading_mutex_init_freertos( mbedtls_threading_mutex_t *mutex )
{
    if( mutex == NULL )
        return;
...
static void threading_mutex_free_freertos( mbedtls_threading_mutex_t *mutex )
{
    if( mutex == NULL )
        return;
...
static int threading_mutex_lock_freertos( mbedtls_threading_mutex_t *mutex )
{
    if( mutex == NULL || ! mutex->is_valid )
        return( MBEDTLS_ERR_THREADING_BAD_INPUT_DATA );

    if( osMutexAcquire( mutex->mutex, portMAX_DELAY ) != osOK )
        return( MBEDTLS_ERR_THREADING_MUTEX_ERROR );

    return( 0 );
}

static int threading_mutex_unlock_freertos( mbedtls_threading_mutex_t *mutex )
{
    if( mutex == NULL || ! mutex->is_valid )
        return( MBEDTLS_ERR_THREADING_BAD_INPUT_DATA );
