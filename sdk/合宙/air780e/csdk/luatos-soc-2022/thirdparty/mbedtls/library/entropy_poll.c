...
#if defined(MBEDTLS_USE_RAND_API_ENTROPY)
//add by xiang
//#warning "Platform entropy sources will use rand() API"
...
{                         
    ...
    *olen = 0;
    ...
    {
        rvalue = rand();    
        memcpy(output, (void *)&rvalue, len);
        *olen = len;
    }
    else 
    {
        for(i=0 ;i < (len/intlen); i++) 
        {
            rvalue = rand();
            memcpy(output+(i*intlen), (void *)&rvalue, intlen);
        }
        
        if((len%intlen) > 0)
        {
            rvalue = rand();
            memcpy(output+(i*intlen), (void *)&rvalue, len%intlen);
        }
        *olen = len;
    }
    return 0;
