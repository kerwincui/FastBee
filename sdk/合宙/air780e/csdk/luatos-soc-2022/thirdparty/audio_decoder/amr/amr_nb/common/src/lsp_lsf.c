...
------------------------------------------------------------------------------
 ...
 The transformation from lsp[i] to lsf[i] is approximated by a look-up lsp_table
 and interpolation.
...
)
{
    ...
    for (i = 0; i < m; i++)
    {
        ...
        // lsp[i] = lsp_table[ind]+ ((lsp_table[ind+1]-lsp_table[ind])*offset) / 256

        L_tmp = L_mult (sub (lsp_table[ind + 1], lsp_table[ind]), offset);
        lsp[i] = add (lsp_table[ind], extract_l (L_shr (L_tmp, 9)));
...
)
{
    ...
    for (i = 0; i < m; i++)
    {
        ...
        L_tmp = ((Word32)(lsp_table[ind + 1] - lsp_table[ind]) * offset) >> 8;
        lsp[i] = lsp_table[ind] + (Word16) L_tmp;
...
------------------------------------------------------------------------------
 ...
 Global Variables Used:
    lsp_table = cosine lsp_table
    slope = lsp_table to used to calculate inverse cosine
...
------------------------------------------------------------------------------
 ...
 The transformation from lsf[i] to lsp[i] is approximated by a look-up lsp_table
 and interpolation.
...
)
{
    ...
    ind = 63;                        // begin at end of lsp_table -1

    for (i = m - 1; i >= 0; i--)
    {
        // find value in lsp_table that is just greater than lsp[i]

        while (sub (lsp_table[ind], lsp[i]) < 0)
        {
            ind--;

        }

        // acos(lsp[i])= ind*256 + ( ( lsp[i]-lsp_table[ind] ) *
           slope[ind] )/4096

        L_tmp = L_mult (sub (lsp[i], lsp_table[ind]), slope[ind]);
        //(lsp[i]-lsp_table[ind])*slope[ind])>>12
...
)
{
    ...
    for (i = m - 1; i >= 0; i--)
    {
        ...
        L_tmp = (Word32)(temp - lsp_table[ind]) * slope[ind];
