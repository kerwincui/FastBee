...
static int lfs_bd_read(lfs_t *lfs,
    ...
    if (block >= lfs->cfg->block_count ||
        ...
        //ECPLAT_PRINTF(UNILOG_LFS, lfs_bd_read_0, P_WARNING, "LFS corrupt detect!");
#endif
...
static int lfs_bd_flush(lfs_t *lfs,
    ...
    if (pcache->block != LFS_BLOCK_NULL && pcache->block != LFS_BLOCK_INLINE) {
        ...
        if (validate) {
            ...
            if (res != LFS_CMP_EQ) {
#ifdef LFS_DEBUG_TRACE
                ECPLAT_PRINTF(UNILOG_LFS, lfs_bd_flush_0, P_WARNING, "LFS corrupt detect!");
#endif
                return LFS_ERR_CORRUPT;
            }
        }
...
#ifdef LFS_DEBUG_TRACE
        ECPLAT_PRINTF(UNILOG_LFS, lfs_dir_getread_0, P_WARNING, "LFS corrupt detect!");
#endif
        return LFS_ERR_CORRUPT;
    }
...
#else
            lfs_size_t diff = 0, entrySize = lfs_tag_dsize(tag);

            // crc the entry first, hopefully leaving it in the cache
            for (lfs_off_t j = sizeof(tag); j < entrySize; j += diff) {

                uint8_t dat[8];
                diff = lfs_min(entrySize - j, sizeof(dat));

                err = lfs_bd_read(lfs,
                        NULL, &lfs->rcache, lfs->cfg->block_size,
                        dir->pair[0], off+j, &dat, diff);
                if (err) {
                    if (err == LFS_ERR_CORRUPT) {
                        dir->erased = false;
                        break;
...
#ifdef LFS_DEBUG_TRACE
    ECPLAT_PRINTF(UNILOG_LFS, lfs_dir_fetchmatch_0, P_ERROR, "Corrupted dir pair at %x %x", dir->pair[0], dir->pair[1]);
#endif
...
static int lfs_dir_commitcrc(lfs_t *lfs, struct lfs_commit *commit) {
    ...
    while (off < end) {
        ...
        if (crc != 0) {
#ifdef LFS_DEBUG_TRACE
            ECPLAT_PRINTF(UNILOG_LFS, lfs_dir_commitcrc_0, P_WARNING, "LFS corrupt detect!");
#endif
            return LFS_ERR_CORRUPT;
        }
    ...
    while (true) {
        {
            ...
            if (err) {
                if (err == LFS_ERR_CORRUPT) {
#ifdef LFS_DEBUG_TRACE
                    ECPLAT_PRINTF(UNILOG_LFS, lfs_dir_compact_0, P_WARNING, "LFS corrupt detect!");
#endif
                    goto relocate;
                }
                return err;
            ...
            if (err) {
                if (err == LFS_ERR_CORRUPT) {
#ifdef LFS_DEBUG_TRACE
                    ECPLAT_PRINTF(UNILOG_LFS, lfs_dir_compact_1, P_WARNING, "LFS corrupt detect!");
#endif
                    goto relocate;
                }
                return err;
            ...
            if (err) {
                if (err == LFS_ERR_CORRUPT) {
#ifdef LFS_DEBUG_TRACE
                    ECPLAT_PRINTF(UNILOG_LFS, lfs_dir_compact_2, P_WARNING, "LFS corrupt detect!");
#endif
                    goto relocate;
                }
                return err;
            ...
            if (!lfs_pair_isnull(dir->tail)) {
                ...
                if (err) {
                    if (err == LFS_ERR_CORRUPT) {
#ifdef LFS_DEBUG_TRACE
                        ECPLAT_PRINTF(UNILOG_LFS, lfs_dir_compact_3, P_WARNING, "LFS corrupt detect!");
#endif
                        goto relocate;
                    }
                    return err;
            ...
            if (!lfs_gstate_iszero(&delta)) {
                ...
                if (err) {
                    if (err == LFS_ERR_CORRUPT) {
#ifdef LFS_DEBUG_TRACE
                        ECPLAT_PRINTF(UNILOG_LFS, lfs_dir_compact_4, P_WARNING, "LFS corrupt detect!");
#endif
                        goto relocate;
                    }
                    return err;
            ...
            if (err) {
                if (err == LFS_ERR_CORRUPT) {
#ifdef LFS_DEBUG_TRACE
                    ECPLAT_PRINTF(UNILOG_LFS, lfs_dir_compact_5, P_WARNING, "LFS corrupt detect!");
#endif
                    goto relocate;
                }
                return err;
    ...
    while (true) {
...

        {
            ...
            if (err) {
                if (err == LFS_ERR_CORRUPT) {
#ifdef LFS_DEBUG_TRACE
                    ECPLAT_PRINTF(UNILOG_LFS, lfs_ctz_extend_0, P_WARNING, "LFS corrupt detect!");
#endif
                    goto relocate;
                }
                return err;
            ...
            if (noff != lfs->cfg->block_size) {
                for (lfs_off_t i = 0; i < noff; i++) {
                    ...
                    if (err) {
                        if (err == LFS_ERR_CORRUPT) {
#ifdef LFS_DEBUG_TRACE
                            ECPLAT_PRINTF(UNILOG_LFS, lfs_ctz_extend_1, P_WARNING, "LFS corrupt detect!");
#endif
                            goto relocate;
                        }
                        return err;
            ...
            for (lfs_off_t i = 0; i < skips; i++) {
                ...
                if (err) {
                    if (err == LFS_ERR_CORRUPT) {
#ifdef LFS_DEBUG_TRACE
                        ECPLAT_PRINTF(UNILOG_LFS, lfs_ctz_extend_2, P_WARNING, "LFS corrupt detect!");
#endif
                        goto relocate;
                    }
                    return err;
...
#ifdef LFS_DEBUG_TRACE
    file->name = lfs_malloc(LFS_NAME_MAX+1);
    if(file->name)
    {
        strncpy(file->name, path, LFS_NAME_MAX+1);
    }
    else
    {
        ECPLAT_PRINTF(UNILOG_LFS, lfs_file_opencfg, P_WARNING, "Can't malloc for name field");
    }
#else
    file->name = NULL;
#endif
...
static int lfs_file_relocate(lfs_t *lfs, lfs_file_t *file) {
    ...
    while (true) {
        ...
        if (err) {
            if (err == LFS_ERR_CORRUPT) {
#ifdef LFS_DEBUG_TRACE
                ECPLAT_PRINTF(UNILOG_LFS, lfs_file_relocate_0, P_WARNING, "LFS corrupt detect!");
#endif
                goto relocate;
            }
            return err;
        ...
        for (lfs_off_t i = 0; i < file->off; i++) {
            ...
            if (err) {
                if (err == LFS_ERR_CORRUPT) {
#ifdef LFS_DEBUG_TRACE
                    ECPLAT_PRINTF(UNILOG_LFS, lfs_file_relocate_1, P_WARNING, "LFS corrupt detect!");
#endif
                    goto relocate;
                }
                return err;
        ...
        if (!(file->flags & LFS_F_INLINE)) {
            ...
            while (true) {
                ...
                if (err) {
                    if (err == LFS_ERR_CORRUPT) {
#ifdef LFS_DEBUG_TRACE
                        ECPLAT_PRINTF(UNILOG_LFS, lfs_file_flush_0, P_WARNING, "LFS corrupt detect!");
#endif
                        goto relocate;
                    }
                    return err;
    ...
    while (nsize > 0) {
        ...
        while (true) {
            ...
            if (err) {
                if (err == LFS_ERR_CORRUPT) {
#ifdef LFS_DEBUG_TRACE
                    ECPLAT_PRINTF(UNILOG_LFS, lfs_file_write_0, P_WARNING, "LFS corrupt detect!");
#endif
                    goto relocate;
                }
                file->flags |= LFS_F_ERRED;
...
int lfs_fs_traverse(lfs_t *lfs, int (*cb)(void *, lfs_block_t), void *data) {
    return lfs_fs_rawtraverse(lfs, cb, data, true);
}
